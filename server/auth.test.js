...
async function binaryApi(baseUrl, pathname, { method = "GET", body, cookie, contentType } = {}) {
  const response = await fetch(`${baseUrl}${pathname}`, {
    method,
    headers: {
...
test("v0.8 accounts protect identities and direct conversations", async () => {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "mayak-v08-test-"));
  const port = await freePort();
  const baseUrl = `http://127.0.0.1:${port}`;
  const child = spawn(process.execPath, ["--no-warnings=ExperimentalWarning", serverFile], {
    env: { ...process.env, HOST: "127.0.0.1", PORT: String(port), MAYAK_DATA_DIR: dataDir },
    stdio: "ignore"
  });
  ...
  try {
    ...
    assert.equal(health.payload.version, "0.8");
    assert.equal(health.payload.auth, "sessions");

    const anonymousHistory = await api(baseUrl, "/api/messages?chatId=live");
    ...
    assert.match(avatarPayload.user.avatarUrl, new RegExp(`/api/users/${roman.payload.user.id}/avatar\\?v=`));

    const avatarDownload = await binaryApi(baseUrl, avatarPayload.user.avatarUrl, { cookie: roman.cookie });
    assert.equal(avatarDownload.response.status, 200);
    assert.equal(avatarDownload.response.headers.get("content-type"), "image/png");
    assert.deepEqual(avatarDownload.bytes, avatarBytes);

    const profileUpdate = await api(baseUrl, "/api/auth/me", {
      method: "PATCH",
      cookie: roman.cookie,
      body: { name: "Роман Орендаренко", bio: "Новая строка профиля", deviceName: "Mac Романа" }
    });
    ...
    const duplicate = await api(baseUrl, "/api/auth/register", {
      method: "POST",
      body: { name: "Другой Роман", handle: "roman", password: "another-pass", deviceId: "other" }
    });
    assert.equal(duplicate.status, 409);

    const kolya = await api(baseUrl, "/api/auth/register", {
      method: "POST",
      body: { name: "Коля", handle: "@kolya", password: "kolya-pass-08", deviceId: "kolya-iphone", deviceName: "iPhone Коли" }
    });
    assert.equal(kolya.status, 201);

    const arina = await api(baseUrl, "/api/auth/register", {
      method: "POST",
      body: { name: "Арина", handle: "@arina", password: "arina-pass-08", deviceId: "arina-phone", deviceName: "Телефон Арины" }
    });
    ...
    assert.equal(direct.payload.conversation.peer.id, kolya.payload.user.id);
    ...
    const kolyaHistory = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: kolya.cookie });
    assert.equal(kolyaHistory.status, 200);
    assert.equal(kolyaHistory.payload.messages.length, 1);

    const arinaHistory = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: arina.cookie });
    assert.equal(arinaHistory.status, 403);

    const forbiddenDelete = await api(baseUrl, `/api/messages/${encodeURIComponent(spoofed.payload.message.id)}`, {
      method: "DELETE",
      cookie: kolya.cookie
    });
    ...
    const deletedMessage = await api(baseUrl, `/api/messages/${encodeURIComponent(spoofed.payload.message.id)}`, {
      method: "DELETE",
      cookie: roman.cookie
    });
    assert.equal(deletedMessage.status, 200);
    assert.equal(deletedMessage.payload.deleted.messageId, spoofed.payload.message.id);

    const historyAfterDelete = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: kolya.cookie });
    ...
    const forbiddenClear = await api(baseUrl, `/api/conversations/${encodeURIComponent(chatId)}/messages`, {
      method: "DELETE",
      cookie: arina.cookie
    });
    assert.equal(forbiddenClear.status, 403);

    const clearedForRoman = await api(baseUrl, `/api/conversations/${encodeURIComponent(chatId)}/messages`, {
      method: "DELETE",
      cookie: roman.cookie
    });
    assert.equal(clearedForRoman.status, 200);

    const romanClearedHistory = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: roman.cookie });
    const kolyaUnclearedHistory = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: kolya.cookie });
    ...
    const romanAfterClear = await api(baseUrl, `/api/messages?chatId=${encodeURIComponent(chatId)}`, { cookie: roman.cookie });
    assert.equal(romanAfterClear.payload.messages.length, 1);
    assert.equal(romanAfterClear.payload.messages[0].text, "Новое сообщение после очистки");

    const logout = await api(baseUrl, "/api/auth/logout", { method: "POST", cookie: roman.cookie, body: {} });
    assert.equal(logout.status, 200);
    const afterLogout = await api(baseUrl, "/api/auth/me", { cookie: roman.cookie });
